...
class DataService {
  ...
  getDataByYear() {
    ...
    let sortedData = [];
    //   {"Year":2012, "1":0.5, "2":0.5, "3":0.5, "4":0.5, "5":0.5,  "6":0.5, "7":0.5, "8":0.5, "9":0.5, "10":0.5, "11":0.5, "12":0.5, "Total":0.5,}
    // ];
    data.forEach(function (item){
      // sortedData[item.Year][item.Month] = item.Value;
      sortedData = this.findOrCreateYear(sortedData, item);
    }.bind(this));
    console.log("sortedData!",sortedData);
    return data;
  }
  findOrCreateYear(sortedData, item) {
    console.log("findOrCreateYear!", item.Year);
    let found = false;
    sortedData.forEach(function(dataItem){
      if(dataItem.Year == item.Year) {
        console.log("Found:", item.Year);
        found = true;
      }
    });
    if (found == false) {
      console.log("make item.Year:",item.Year);
      sortedData.push({"Year":item.Year});
    }
    return sortedData;
  }
  addItem(item) {
